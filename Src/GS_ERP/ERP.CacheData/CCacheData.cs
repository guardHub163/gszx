...
namespace CZZD.ERP.CacheData
{
    public class CCacheData
    {
        ...
        private static DataTable _baseProductName = null;
        ...
        /// 所有常用商品输入内容
        /// </summary>
        public static DataTable BaseProductName
        {
            get
            {
                if (_baseProductName == null)
                {
                    _baseProductName = bCommon.GetProductNames("").Tables[0]; ;
                }
                return CCacheData._baseProductName;
            }
            set { CCacheData._baseProductName = value; }
        }
        ...
        /// <param name="code"></param>
        /// <returns></returns>
        public static string[] GetBaseProductNamesData()
        {

            string[] str = new string[CCacheData.BaseProductName.Rows.Count];
            int i = 0;
            foreach (DataRow row in CCacheData.BaseProductName.Rows)
            {
                str[i++] = CConvert.ToString(row["GOODS_SN"]) + "|" + CConvert.ToString(row["GOODS_NAME"]);
            }
            return str;
        }
