...
namespace CZZD.ERP.SQLServerDAL
{
    public class CommonManage : ICommon
    {
        ...
        public BaseMaster GetBaseMaster(string tableName, string code, string strWhere)
        {
            ...
            {
                strSql.Append("select CODE,NAME from base_customer_view");
                strSql.AppendFormat(" where CODE = @CODE  ", CConstant.DELETE);
                if (strWhere.Trim() != "")
                {
                    strSql.Append(" and " + strWhere);
                }
        ...
        public int GetMasterRecordCount(string tableName, string code, string name, string strWhere)
        {
            ...
            else if ("CUSTOMER".Equals(tableName)) 
            {
                ...
                strSql.AppendFormat(" where CODE like @CODE and NAME like @NAME ", CConstant.DELETE);
            }
            else
            {
                strSql.AppendFormat("select count(1) from BASE_{0} ", tableName);
        ...
        public DataSet GetMasterDataList(string tableName, string code, string name, string strWhere, int startIndex, int endIndex)
        {
            ...
            {
                strSql.Append("select * from base_customer_view");
                strSql.AppendFormat(" where CODE like @CODE and NAME like @NAME ", CConstant.DELETE);
            }
            else
            {
        ...
        public DataSet GetMasterList(string tableName, string code, string name, string strWhere)
        {
            ...
            {
                strSql.Append("select * from base_customer_view");
                strSql.AppendFormat(" where CODE like @CODE and CODE like @NAME ", CConstant.DELETE);
            }
            else
            {
        ...
        /// 所有商品输入内容
        /// </summary>
        public DataSet GetProductNames(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" SELECT * FROM BASE_PRODUCT WHERE 1=1 ");
            if (!string.IsNullOrEmpty(strWhere))
            {
                strSql.AppendFormat(" AND {0} ", strWhere);
            }
            strSql.Append(" ORDER BY GOODS_SN");
            return DbHelperSQL.Query(strSql.ToString());
        }
